...
- ps 指令详解
    - 指令：ps –aux|grep xxx ，比如我看看有没有 sshd 服务（grep用来做过滤，现在只要sshd服务）
    - 指令说明
        - System V 展示风格
        - USER：用户名称
        - PID：进程号
...
- 案例
    - 以全格式显示当前所有的进程，查看进程的父进程。
        - 方式一：ps -ef | more（ps -ef 是以全格式显示当前所有的进程，管道符|后面跟more，代表分页查看）
        - 方式二：ps -ef|grep xxx
...
- 案例
    - 踢掉某个非法登录用户
        - 输入ps -aux | grep sshd，先搜索到远程登录用的进程，获取它的进程号，例如jack，它的进程号是4010
        - 输入kill 4010，杀死指定进程号的进程
    - 终止远程登录服务 sshd, 在适当时候再次重启 sshd 服务（例如：在做系统维护时，不能有人登录进系统）
        - ps -aux | grep sshd，获取sshd的进程号，找/usr/sbin/sshd -D的，例如进程号是3908
        - kill 3908，杀死sshd的进程，所有登录用户都退出远程登录（杀死时不会有输出，再次kill时会提示没有该进程）
    - 终止多个 gedit 编辑器（killall , 通过进程名称来终止进程）（如需要杀死整个服务，服务有很多子进程，可以杀死父进程来批量杀死子进程）
        - killall gedit（通过一个进程名称，杀死所有的编辑器进程）
    - 强制杀掉一个终端
        - ps -aux | grep bash，查询终端shell的进程，找/bin/bash的，有多少个，就有多个终端，例如找一个进程号为4090的
        - kill 4090，发现杀不掉，因为系统任务终端是一个比较重要的进程，忽略了kill
        - kill -9 4090，加上-9参数，强制杀死即可
...
- 安装
    - yum -y install psmisc，centos默认没有安装pstree，需要用yum进行安装
...
- 细节
    - 关闭或者启用防火墙后，立即生效。[telnet 测试某个端口即可]，例如windows上输入telnet 192.168.184.130 22
    - 这种方式只是临时生效，当重启系统后，还是回归以前对服务的设置。 如果希望设置某个服务自启动或关闭永久生效，要使用 chkconfig 指令
    
### 查看服务名

- 方式一：使用 setup -> 系统服务 就可以看到。
- 方式二：/etc/init.d/服务名称
...
- 开机的流程说明
    - 开机 => BIOS => /boo => init进程1 => 运行级别 => 运行对应级别的服务
...
- 语法
    ...
    - chkconfig --level 5 服务名 on/off，查看指定服务在某个运行级别
...
- 案例
    ...
    - 将 sshd 服务在运行级别 5 下设置为不自动启动，看看有什么效果？
        - 输入：chkconfig --level 5 sshd off，将sshd在5运行级别下，不自启动，就是关闭
    - 当运行级别为 5 时，关闭防火墙
...
- 选项说明
    - `-d` 秒数：指定top命令每隔几秒更新。默认是3秒在top命令的交互模式当中执行的命令
    - `-i` 使top不显示任何闲置或者僵死进程
...
- 案例
    - 监视特定用户
        - top：输入此命令，按回车键，查看执行的进程。 u：然后输入“u”回车，再输入用户名（例如root），即可
    - 终止指定的进程
...
- 安装
    - centos默认没有安装netstat指令，如果出现command not found，使用yum安装即可：yum install net-tools
