...
因安卓自带的Switch不符合UI风格，所以应要求做一个类似微信的开关，颜色支持自定义。

自定义完后，不算复杂，主要是有个计算的思路，其实绘制只有2部分。

1. 白色小圆的位置，通过值变化动画，将小圆的x坐标从左到右移动。（数学计算）
2. 背景绘制圆弧矩形，根据开关变化，通过颜色估值器，从对应的背景颜色平滑过渡到下一个状态的背景色。

![仿微信开关.png](https://upload-images.jianshu.io/upload_images/1641428-bd828a0d2c47dada.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/300)
